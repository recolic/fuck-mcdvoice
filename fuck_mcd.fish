...
####### First query to construct cookie
...
# get Index.aspx access first!

set nextHop_postfix (grep 'href="/Index.aspx?c=' $tmpFile | sed 's/^.*Index.aspx//g' | sed 's/".*$//g')
set -g nextHop "https://www.mcdvoice.com/Index.aspx$nextHop_postfix"
...
curl -vv "$nextHop" --cookie "$cookie" > $tmpFile 2>&1
